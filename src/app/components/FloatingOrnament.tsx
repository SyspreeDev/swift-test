...
export function FloatingOrnament({
  ...
  // FIX: throttled mousemove — was raw setState on every pixel
  useEffect(() => {
  ...
  // FIX: throttled scroll + getBoundingClientRect was forced layout on EVERY scroll tick
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (ornamentRef.current) observer.observe(ornamentRef.current);
    return () => { if (ornamentRef.current) observer.unobserve(ornamentRef.current); };
  }, []);

  const getPositionStyles = (): React.CSSProperties => {
    const base: React.CSSProperties = { top: '50%' };
    if (position === 'left')
      return { ...base, left: 0,     transform: `translate(${-150 + mousePosition.x}px, ${offsetY}) translateY(${scrollY}px)`, transition: 'transform 0.5s ease-out' };
    if (position === 'right')
      return { ...base, right: 0,    transform: `translate(${150  + mousePosition.x}px, ${offsetY}) translateY(${scrollY}px)`, transition: 'transform 0.5s ease-out' };
    return   { ...base, left: '50%', transform: `translate(calc(-50% + ${mousePosition.x}px), ${offsetY}) translateY(${scrollY}px)`, transition: 'transform 0.5s ease-out' };
  };

  // FIX: Don't render DOM at all when invisible — saves GPU compositing layers
  if (!isVisible) {
    return (
      <div
        ref={ornamentRef}
        className="absolute pointer-events-none hidden lg:block"
        style={{ ...getPositionStyles(), width: `${currentSize.main + 200}px`, height: `${currentSize.main + 200}px`, opacity: 0 }}
      />
    );
  }
  ...
  return (
    <div
      ...
      style={{
        ...
        filter: 'blur(0.5px)',
        zIndex: 0,
        // FIX: promote to own compositing layer so CSS animations don't trigger repaints
    ...
    >
      ...
      <div
        className="absolute inset-0"
        style={{ animation: `spin-ornament ${currentSpeed.main}s linear infinite`, transformStyle: 'preserve-3d' }}
      >
        <div className="relative w-full h-full">
          {/* Central ornament */}
          <div
            ...
            style={{ width: currentSize.main, height: currentSize.main }}
          >
            <svg className="w-full h-full" viewBox="0 0 613.215 613.234" fill="none" preserveAspectRatio="xMidYMid meet">
              <g>
                <path d={svgPaths.p5501680}  fill="#007969" />
                <path d={svgPaths.p283f4d00} fill="#007969" />
                <path d={svgPaths.p2b80a100} fill="#007969" />
                <path d={svgPaths.p28198470} fill="#007969" />
                <path d={svgPaths.p2b88e00}  fill="#007969" />
                <g><path d={svgPaths.p17f3f000} fill="#007969" /><path d={svgPaths.p158a0c80} fill="#007969" /></g>
                <g><path d={svgPaths.p37eb4400} fill="#007969" /><path d={svgPaths.p17b28640} fill="#007969" /></g>
                <g><path d={svgPaths.p242f1a80} fill="#007969" /><path d={svgPaths.p14229700} fill="#007969" /></g>
                <g><path d={svgPaths.p37775500} fill="#007969" /><path d={svgPaths.p3754f370} fill="#007969" /></g>
                <g><path d={svgPaths.p304d1b00} fill="#007969" /><path d={svgPaths.p4364930}  fill="#007969" /></g>
                <g><path d={svgPaths.p283dda80} fill="#007969" /><path d={svgPaths.p361aab40} fill="#007969" /></g>
                <g><path d={svgPaths.p129909c0} fill="#007969" /><path d={svgPaths.p1e2e4c80} fill="#007969" /></g>
                <g><path d={svgPaths.p3d59c600} fill="#007969" /><path d={svgPaths.p3b321480} fill="#007969" /></g>
                <g><path d={svgPaths.p2a159a00} fill="#007969" /><path d={svgPaths.p272f4d00} fill="#007969" /></g>
              </g>
            </svg>
          </div>
          ...
          {[0, 1, 2, 3].map((index) => {
            ...
            return (
              ...
              >
                <div style={{ width: currentSize.satellite, height: currentSize.satellite, animation: `spin-reverse ${currentSpeed.satellite}s linear infinite` }}>
                  <svg className="w-full h-full opacity-60" viewBox="0 0 613.215 613.234" fill="none" preserveAspectRatio="xMidYMid meet">
                    <g>
                      <path d={svgPaths.p5501680}  fill="#007969" />
                      <path d={svgPaths.p283f4d00} fill="#007969" />
                      <path d={svgPaths.p2b80a100} fill="#007969" />
      ...
      <style>{`
        @keyframes spin-ornament  { from { transform: rotate(0deg); }   to { transform: rotate(360deg); } }
        @keyframes spin-reverse   { from { transform: rotate(0deg); }   to { transform: rotate(-360deg); } }
        @keyframes orbit-path     { from { transform: translate(-50%,-50%) rotate(0deg); } to { transform: translate(-50%,-50%) rotate(-360deg); } }
        @keyframes float-particle {
